...
class AP_BNO055_AuxiliaryBus;
class AP_BNO055_AuxiliaryBusSlave;

// enable debug to see a register dump on startup
#define BNO055_DEBUG 0

class AP_InertialSensor_BNO055 : public AP_InertialSensor_Backend
{
    friend AP_BNO055_AuxiliaryBus;
    friend AP_BNO055_AuxiliaryBusSlave;

public:
    virtual ~AP_InertialSensor_BNO055();

    static AP_InertialSensor_BNO055 &from(AP_InertialSensor_Backend &backend) {
        return static_cast<AP_InertialSensor_BNO055&>(backend);
    }
...
private:
    AP_InertialSensor_BNO055(AP_InertialSensor &imu,
    ...
    AP_BNO055_AuxiliaryBus *_auxiliary_bus;
};

class AP_BNO055_AuxiliaryBusSlave : public AuxiliaryBusSlave
{
    friend class AP_BNO055_AuxiliaryBus;
...
protected:
    AP_BNO055_AuxiliaryBusSlave(AuxiliaryBus &bus, uint8_t addr, uint8_t instance);
    int _set_passthrough(uint8_t reg, uint8_t size, uint8_t *out = nullptr);

private:
    const uint8_t _BNO055_addr;
    const uint8_t _BNO055_reg;
    const uint8_t _BNO055_ctrl;
    const uint8_t _BNO055_do;

    uint8_t _ext_sens_data = 0;
};

class AP_BNO055_AuxiliaryBus : public AuxiliaryBus
{
    friend class AP_InertialSensor_BNO055;

public:
    AP_HAL::Semaphore *get_semaphore() override;

protected:
    AP_BNO055_AuxiliaryBus(AP_InertialSensor_BNO055 &backend);
